...
static const bg_parameter_info_t parameters[] =
  {
    ...
    },
    {
      ...
      .val_default = GAVL_VALUE_INIT_STRING("vcd2"),
    ...
    },
    {
      ...
      .val_default = GAVL_VALUE_INIT_STRING("VIDEOCD"),
    },
    {
      .name = "xml_file",
      .long_name = TRS("Xml file"),
      .type = BG_PARAMETER_STRING,
      .val_default = GAVL_VALUE_INIT_STRING("videocd.xml"),
    },
    {
      .name = "bin_file",
      .long_name = TRS("Bin file"),
      .type = BG_PARAMETER_STRING,
      .val_default = GAVL_VALUE_INIT_STRING("videocd.bin"),
    },
    {
      .name = "cue_file",
      .long_name = TRS("Cue file"),
      .type = BG_PARAMETER_STRING,
      .val_default = GAVL_VALUE_INIT_STRING("videocd.cue"),
    },
    CDRDAO_PARAMS,
    { /* End of parameters */ },
  };
...
#define SET_STR(key) if(!strcmp(name, # key)) { vcd->key = gavl_strrep(vcd->key, v->v.str); return; }

static void set_parameter_vcdimager(void * data, const char * name, const gavl_value_t * v)
  {
...
static void parse_output_line(vcdimager_t * vcdimager, char * line)
  ...
  if(!strncmp(line, "<log ", 5))
    ...
    if(!strncmp(start, "warning", 7))
      log_level = GAVL_LOG_WARNING;
    else if(!strncmp(start, "information", 11))
      log_level = GAVL_LOG_INFO;
    else if(!strncmp(start, "error", 5))
      log_level = GAVL_LOG_ERROR;
    ...
    if(!(end = strstr(start, "</log>")))
      return;
    *end = '\0';
    gavl_log(log_level, LOG_DOMAIN, "%s", start);
    }
...
static void run_vcdimager(void * data, const char * directory, int cleanup)
  ...
  while(bg_subprocess_read_line(proc->stderr_fd, &line, &line_alloc, -1))
    ...
    if(line && (*line != '\0'))
      {
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "vcdxgen failed: %s", line);
      err = 1;
      }
    }
  ...
  if(cleanup)
    ...
    for(i = 0; i < vcdimager->num_files; i++)
      ...
      if(!vcdimager->files[i].pp_only)
        {
        gavl_log(GAVL_LOG_INFO, LOG_DOMAIN, "Removing %s", vcdimager->files[i].name);
        remove(vcdimager->files[i].name);
        }
      }
    gavl_log(GAVL_LOG_INFO, LOG_DOMAIN, "Removing %s", xml_file);
    remove(xml_file);
    }
  ...
  if(bg_cdrdao_run(vcdimager->cdr, cue_file) && cleanup)
    {
    gavl_log(GAVL_LOG_INFO, LOG_DOMAIN, "Removing %s", bin_file);
    remove(bin_file);
    gavl_log(GAVL_LOG_INFO, LOG_DOMAIN, "Removing %s", cue_file);
    remove(cue_file);
    }
