 ...
 *
 * $Id: sha1.c,v 1.1 2005-02-17 23:44:44 gmerlin Exp $
 */
...
void bg_cdaudio_sha_init(SHA_INFO *sha_info)
{
...
void bg_cdaudio_sha_update(SHA_INFO *sha_info, const SHA_BYTE *buffer, int count)
{
    int i;
    SHA_LONG clo;
...
void bg_cdaudio_sha_final(unsigned char digest[20], SHA_INFO *sha_info)
{
    int count;
...
void bg_cdaudio_sha_stream(unsigned char digest[20], SHA_INFO *sha_info, FILE *fin)
{
    int i;
    SHA_BYTE data[BLOCK_SIZE];

    bg_cdaudio_sha_init(sha_info);
    while ((i = fread(data, 1, BLOCK_SIZE, fin)) > 0) {
	bg_cdaudio_sha_update(sha_info, data, i);
    }
    bg_cdaudio_sha_final(digest, sha_info);
}

/* print a SHA digest */

void bg_cdaudio_sha_print(unsigned char digest[20])
{
    int i, j;
