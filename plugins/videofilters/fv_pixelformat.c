  ...
  {
    {
      ...
      .val_default = GAVL_VALUE_INIT_STRING("yuv420p8"),
    },
    { /* End of parameters */ },
  };
  ...
  for(i = 0; i < num; i++)
    ...
    if(f != GAVL_PIXELFORMAT_NONE)
      ...
      ret->multi_names_nc[index] =
        gavl_strdup(gavl_pixelformat_to_short_string(f));
      ret->multi_labels_nc[index] =
        gavl_strdup(gavl_pixelformat_to_string(f));
      index++;
      }
    }
...
set_parameter_pixelformat(void * priv, const char * name,
  ...
  if(!strcmp(name, "pixelformat"))
    {
    f = gavl_short_string_to_pixelformat(val->v.str);
    if(vp->pixelformat != f)
      {
      vp->need_restart = 1;
      vp->pixelformat = f;
