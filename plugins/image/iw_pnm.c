  ...
  {
    {
      ...
      .val_default = GAVL_VALUE_INIT_STRING("binary"),
    },
    {
      .name =        "comment",
      ...
      .val_default = GAVL_VALUE_INIT_STRING("Created with gmerlin"),
...
static void set_parameter_pnm(void * p, const char * name,
  ...
  if(!name)
    ...
    {
    if(!strcmp(val->v.str, "binary"))
      pnm->binary = 1;
    else if(!strcmp(val->v.str, "ascii"))
      pnm->binary = 0;
    }
