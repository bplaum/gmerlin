...
static int read_header_bmp(void *priv,const char *filename, gavl_video_format_t * format)
  ...
  if(!p->bmp_file)
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "Cannot open file %s: %s", filename, strerror(errno));
  ...
  if(fread(type, 1, 2, p->bmp_file)!=2)
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "Cannot read File type");
    return 0;
    }
  if(strncmp(type, "BM", 2))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN,"File is no BMP");
    return 0;
    }
  ...
  if ((p->width < 1) || (p->height < 1) || (p->width > 8192) || (p->height > 8192))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN,"Cannot detect image dimensions");
    return 0;
    }
  ...
  if ((p->bitcount != 1) && (p->bitcount != 4) && (p->bitcount != 8) && (p->bitcount != 16) && (p->bitcount != 24) && (p->bitcount != 32))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN,"Depth %d not supported", p->bitcount);
    return 0;
    }
  ...
  gavl_dictionary_set_string(&p->m, GAVL_META_FORMAT, "BMP");
...
static const gavl_dictionary_t * get_metadata_bmp(void * priv)
  {
  bmp_t *p = priv;
  return &p->m;
  }
