...
/* Imported file, original copyright note below */
...
/* helpers */
static tga_result tga_read_rle(tga_image *dest, struct read_struct * s);
...
/* ---------------------------------------------------------------------------
 * Read a Targa image from <buf> to <dest>.
 *
...
static int memread(void * dst, int len, struct read_struct * s)
  {
  int ret;
  if(!s->len)
    return 0;
...
tga_result tga_read_from_memory(tga_image *dest, uint8_t * buf, int len)
{
    ...
    {
        /* read RLE */
        tga_result result = tga_read_rle(dest, &s);
...
static tga_result tga_read_rle(tga_image *dest, struct read_struct * s)
{
    ...
    while ((p_loaded < p_expected) && s->len)
    {
        uint8_t b;
        READ(&b, 1);
        if (b & RLE_BIT)
