...
init_read(v4l2_t * v4l)
  ...
  if (!v4l->buffers)
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "Out of memory");
    return 0;
    }
  return 1;
  }
...
init_mmap(v4l2_t * v4l)
  ...
  if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_REQBUFS, &req))
    ...
    if (EINVAL == errno)
      ...
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "%s does not support "
             "memory mapping", bg_multi_menu_get_selected_name(&v4l->dev));
      return 0;
      }
    else
      {
  ...
  if (req.count < 2)
    ...
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN,
           ...
           bg_multi_menu_get_selected_name(&v4l->dev));
    return 0;
    }
  ...
  for (i = 0; i < v4l->n_buffers; ++i)
    ...
    if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_QBUF, &buf))
      {
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_QBUF failed: %s",
             strerror(errno));
      return 0;
      }
  ...
  if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_STREAMON, &type))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_STREAMON failed: %s",
           strerror(errno));
    return 0;
    }
  return 1;
...
static int read_frame_read(v4l2_t * v4l, gavl_video_frame_t ** frame)
  ...
  if (-1 == read (v4l->fd, v4l->buffers[0].start, v4l->fmt.fmt.pix.sizeimage))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "read failed: %s", strerror(errno));
    switch (errno)
      {
      case EAGAIN:
        return 0;
...
read_frame_v4l(void * priv, gavl_video_frame_t ** frame)
  ...
  for (;;)
    ...
    if (-1 == r)
      ...
      if (EINTR == errno)
        continue;
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "Select failed: %s",
             strerror(errno));
      return 0;
      }
    ...
    if (0 == r)
      {
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "Select timeout");
      return 0;
      }
...
static void set_parameter_v4l(void * priv, const char * name,
  ...
  if(!name)
    ...
    {
    gavl_value_free(&v4l->dev);
    gavl_value_init(&v4l->dev);
    gavl_value_copy(&v4l->dev, val);
    }
  else if(!strcmp(name, "force_rw"))
    v4l->force_rw = val->v.i;
  else if(!strcmp(name, "resolution"))
    {
    if(!strcmp(val->v.str, "qsif"))
      {
      v4l->width  = 160;
      v4l->height = 112;
      v4l->user_resolution = 0;
      }
    else if(!strcmp(val->v.str, "qcif"))
      {
      v4l->width  = 176;
      v4l->height = 144;
      v4l->user_resolution = 0;
      }
    else if(!strcmp(val->v.str, "qvga"))
      {
      v4l->width  = 320;
      v4l->height = 240;
      v4l->user_resolution = 0;
      }
    else if(!strcmp(val->v.str, "sif"))
      {
      v4l->width  = 352;
      v4l->height = 240;
      v4l->user_resolution = 0;
      }
    else if(!strcmp(val->v.str, "cif"))
      {
      v4l->width  = 352;
      v4l->height = 288;
      v4l->user_resolution = 0;
      }
    else if(!strcmp(val->v.str, "vga"))
      {
      v4l->width  = 640;
      v4l->height = 480;
      v4l->user_resolution = 0;
    ...
    {
    v4l->user_width = val->v.i;
    }
  else if(!strcmp(name, "user_height"))
    {
    v4l->user_height = val->v.i;
    }
...
static int open_v4l(void * priv,
  ...
  if(!(dev = bg_multi_menu_get_selected_name(&v4l->dev)))
    return 0;
  
  v4l->fd = bgv4l2_open_device(dev, V4L2_CAP_VIDEO_CAPTURE, &cap);
  ...
  gavl_dictionary_set_string(m, GAVL_META_DEVICE, (char*)cap.card);
  ...
  if((cap.capabilities & V4L2_CAP_STREAMING) &&
    ...
    {
    gavl_log(GAVL_LOG_INFO, LOG_DOMAIN, "Trying read i/o");
  ...
  if(!get_pixelformat(v4l->fd, &v4l->v4l2_pixelformat))
    {
#ifdef HAVE_V4LCONVERT
    gavl_log(GAVL_LOG_INFO, LOG_DOMAIN, "Trying v4lconvert");
    ...
    if(!v4l->converter)
      {
#endif
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "Got no supported pixelformat");
    return 0;
#ifdef HAVE_V4LCONVERT
      }
#endif
  ...
  if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_G_FMT, &v4l->fmt))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_G_FMT failed: %s",
           strerror(errno));
    return 0;
    }
  ...
  if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_S_FMT, &v4l->fmt))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_S_FMT failed: %s",
           strerror(errno));
    return 0;
    }
  ...
  if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_G_PARM, &param))
    ...
    if (EINVAL == errno)
      {
      gavl_log(GAVL_LOG_WARNING, LOG_DOMAIN, "%s does not support "
             "VIDIOC_G_PARAM", bg_multi_menu_get_selected_name(&v4l->dev));
      }
    }
  else
    {
  ...
  if(default_fps < 0.0)
    ...
    if (-1 == bgv4l2_ioctl (v4l->fd, VIDIOC_S_PARM, &param))
      ...
      if (EINVAL == errno)
        {
        gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "%s does not support "
               "VIDIOC_S_PARAM", bg_multi_menu_get_selected_name(&v4l->dev));
        }
      else
        {
        gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_S_PARAM failed: %s",
               strerror(errno));
        }
      }
    else
