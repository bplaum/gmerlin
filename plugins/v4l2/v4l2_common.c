  ...
  *ret = realloc(*ret, ( (*num)+2 ) * sizeof(**ret));
  ...
  switch(ctrl->type)
    ...
    case V4L2_CTRL_TYPE_INTEGER:
      ...
      gavl_value_set_int(&info->val_min, ctrl->minimum);
      gavl_value_set_int(&info->val_max, ctrl->maximum);
      gavl_value_set_int(&info->val_default, ctrl->default_value);
      break;
    case V4L2_CTRL_TYPE_INTEGER64:
      info->type = BG_PARAMETER_INT;
      break;
  ...
  for(i = 0; i < num_controls; i++)
    ...
    if(!strcmp(name, (char*)controls[i].name))
      ...
      if(!val)
        ...
        {
        // fprintf(stderr, "Set parameter: %s %d [%d]", name, val->v.i, v4l->controls[i].id);
        ctrl.value = val->v.i;
        }
      ctrl.id = controls[i].id;
      
      if(bgv4l2_ioctl(fd, VIDIOC_S_CTRL, &ctrl))
        gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_S_CTRL Failed");
      return 1;
      }
    }
  return 0;
  ...
  for(i = 0; i < num_controls; i++)
      ...
      if(!strcmp(name, (char*)controls[i].name))
        ...
        if(!bgv4l2_ioctl(fd, VIDIOC_G_CTRL, &ctrl))
          ...
          gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_G_CTRL Failed");
        return 0;
        }
      }
  return 0;
...
int bgv4l2_open_device(const char * device, int capability,
  ...
  if (-1 == bgv4l2_ioctl (ret, VIDIOC_QUERYCAP, cap))
    ...
    if (EINVAL == errno)
      {
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "%s is no V4L2 device",
               device);
      goto fail;
      }
    else
      {
      gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "VIDIOC_QUERYCAP failed: %s",
             strerror(errno));
      goto fail;
      }
    }
  ...
  if (!(cap->capabilities & capability))
    {
    gavl_log(GAVL_LOG_ERROR, LOG_DOMAIN, "%s is no video %s device",
           device,
