...
static int write_info_chunk(gavl_io_t * output, gavl_dictionary_t * m)
  ...
  info.IART = gavl_dictionary_get_string(m, GAVL_META_ARTIST);
  info.INAM = gavl_dictionary_get_string(m, GAVL_META_TITLE);
  info.ICMT = gavl_dictionary_get_string(m, GAVL_META_COMMENT);
  info.ICOP = gavl_dictionary_get_string(m, GAVL_META_COPYRIGHT);
  info.IGNR = gavl_dictionary_get_string(m, GAVL_META_GENRE);
...
static void set_parameter_wav(void * data, const char * name,
  ...
  if(!strcmp(name, "write_info_chunk"))
    wav->write_info_chunk = v->v.i;
  if(!strcmp(name, "format"))
    {
    if(!strcmp(v->v.str, "wav"))
      wav->file_format = FORMAT_WAV;
    if(!strcmp(v->v.str, "raw"))
...
static int open_io_wav(void * data, gavl_io_t * io,
  ...
  if(metadata)
    gavl_dictionary_copy(&wav->metadata, metadata);
...
static int close_wav(void * data, int do_delete)
  ...
  gavl_dictionary_free(&wav->metadata);
