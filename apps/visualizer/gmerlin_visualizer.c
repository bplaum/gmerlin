...
static gboolean configure_callback(GtkWidget * w, GdkEventConfigure *event,
  ...
  gdk_window_get_root_origin(gtk_widget_get_window(win->current_window->window),
                             &win->x, &win->y);
  return FALSE;
  }
...
static void attach_toolbar(visualizer_t * v, window_t * win)
  ...
  if(v->toolbar_location == TOOLBAR_LOCATION_TOP)
    gtk_widget_set_valign(win->box, GTK_ALIGN_START);
  else
    gtk_widget_set_valign(win->box, GTK_ALIGN_END);
...
static void open_audio(visualizer_t * v)
  ...
  if(v->audio_open)
    {
    v->src = v->ra_plugin->get_audio_source(v->ra_handle->priv);
...
static void plugin_window_init(plugin_window_t * win, visualizer_t * v)
  ...
  win->close_button = gtk_button_new_with_mnemonic("_Close");
  ...
  bg_gtk_table_attach(table, win->close_button, 0, num_columns,
                      row, row+1, 0, 0);
...
static void window_init(visualizer_t * v,
  ...
  gtk_widget_set_halign(w->box, GTK_ALIGN_FILL);
  gtk_widget_set_valign(w->box, GTK_ALIGN_START);
  
  gtk_widget_show(w->box);
  gtk_widget_show(w->socket);

  table = gtk_overlay_new();

  gtk_container_add(GTK_CONTAINER(table), w->socket);
  gtk_overlay_add_overlay(GTK_OVERLAY(table), w->box);
...
static void set_general_parameter(void * data, const char * name,
  ...
  if(!strcmp(name, "toolbar_location"))
    ...
    {
    if(!strcmp(val->v.str, "mouse"))
      {
      v->toolbar_trigger =
        TOOLBAR_TRIGGER_MOUSE;
      }
    else if(!strcmp(val->v.str, "key"))
      {
      v->toolbar_trigger = TOOLBAR_TRIGGER_KEY;
      }
    else if(!strcmp(val->v.str, "mousekey"))
      {
      v->toolbar_trigger =
        TOOLBAR_TRIGGER_MOUSE |
        TOOLBAR_TRIGGER_KEY;
...
bg_parameter_info_t parameters[] =
  {
    ...
    },
    {
      ...
      .type =       BG_PARAMETER_STRINGLIST,
      .val_default = GAVL_VALUE_INIT_STRING("mousekey"),
...
static gboolean idle_func(void * data)
  ...
  if(v->audio_open)
    {
    if(gavl_audio_source_read_frame(v->src, &frame) != GAVL_SOURCE_OK)
      return FALSE;
    
    bg_visualizer_update(v->visualizer, frame);
    
    if(v->toolbar_visible)
      bg_gtk_vumeter_update(v->vumeter, frame);
...
static visualizer_t * visualizer_create()
  ...
  ret->log_window = bg_gtk_log_window_create(TR("Gmerlin visualizer"));
  ...
  gtk_widget_set_halign(ret->fps, GTK_ALIGN_START);
  gtk_widget_set_valign(ret->fps, GTK_ALIGN_CENTER);
  ...
  ret->visualizer = bg_visualizer_create(bg_plugin_reg);
  ...
  bg_gtk_table_attach(main_table, table, 0, 1, 0, 1, 0, 0);
  ...
  bg_gtk_table_attach(main_table, box, 0, 1, 1, 2, 0, 0);
  ...
  if(ret->width && ret->height)
    gdk_window_move_resize(gtk_widget_get_window(ret->current_window->window),
                           ret->x, ret->y,
...
static void visualizer_destroy(visualizer_t * v)
  ...
  char * tmp_path;
  get_config(v);
  tmp_path =  bg_search_file_write("visualizer", "cfg.xml");
  bg_cfg_registry_save(v->cfg_reg, tmp_path);
  if(tmp_path)
    free(tmp_path);
  gavl_dictionary_free(&v->m);
  free(v);
  }
...
int main(int argc, char ** argv)
  ...
  bg_cfg_registry_init("visualizer");
  bg_plugins_init();
