...
static void realize_callback(GtkWidget * w,
  ...
  gdk_window_set_cursor(gtk_widget_get_window(u->widget), u->cursor);
  }
...
bg_gtk_urllink_t * bg_gtk_urllink_create(const char * text, const char * url)
  ...
  if(!ret->browser_command)
    ...
    ret->cursor = gdk_cursor_new_for_display(gdk_display_get_default(), GDK_HAND2);
