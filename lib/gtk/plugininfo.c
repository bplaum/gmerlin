...
pluginwindow_create(const char * title, const char * properties, const char * description)
  ...
  bg_gtk_table_attach_defaults(table, frame, 0, 1, 0, 1);
  ...
  bg_gtk_table_attach_defaults(table, frame, 0, 1, 1, 2);
...
type_names[] =
  {
    ...
    { TRS("Text subtitle exporter"),  BG_PLUGIN_ENCODER_TEXT },
    { TRS("Overlay subtitle exporter"),  BG_PLUGIN_ENCODER_OVERLAY },
...
void bg_gtk_plugin_info_show(const bg_plugin_info_t * info, GtkWidget * parent)
  ...
  char * extensions_string = gavl_value_join_arr(&info->extensions_val, ", ");
  char * protocols_string = gavl_value_join_arr(&info->protocols_val, ", ");
  ...
  if(protocols_string)
    free(protocols_string);

  if(extensions_string)
    free(extensions_string);

  if(mimetypes_string)
    free(mimetypes_string);
