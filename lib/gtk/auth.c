...
static userpass_win * userpass_win_create(const char * resource)
  ...
  gtk_widget_show(w);
  bg_gtk_table_attach(table, w, 0, 3, 0, 1, 0, 0);
  
  w = gtk_label_new(resource);
  gtk_widget_show(w);
  bg_gtk_table_attach(table, w, 0, 3, 1, 2, 0, 0);
 
#ifdef GTK_STOCK_DIALOG_AUTHENTICATION 
  w = gtk_image_new_from_stock(GTK_STOCK_DIALOG_AUTHENTICATION,
                               GTK_ICON_SIZE_DIALOG);
  gtk_widget_show(w);
  bg_gtk_table_attach(table, w, 0, 1, 2, 5, 0, 0);
#endif

  w = gtk_label_new(TR("Username"));
  gtk_widget_show(w);
  bg_gtk_table_attach(table, w, 1, 2, 2, 3, 0, 0);
  bg_gtk_table_attach_defaults(table, ret->user, 2, 3, 2, 3);

  w = gtk_label_new(TR("Password"));
  gtk_widget_show(w);
  bg_gtk_table_attach(table, w, 1, 2, 3, 4, 0, 0);
  bg_gtk_table_attach_defaults(table, ret->pass, 2, 3, 3, 4);

  bg_gtk_table_attach_defaults(table, ret->save_auth, 1, 3, 4, 5);
  ...
  bg_gtk_box_pack_start(box, ret->cancel_button, 1);
  bg_gtk_box_pack_start(box, ret->ok_button, 1);
  gtk_widget_show(box);
  
  bg_gtk_box_pack_start(mainbox, box, 0);
